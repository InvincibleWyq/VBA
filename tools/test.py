...
from mmaction.apis import (init_random_seed, multi_gpu_meta_test,
...
from mmaction.utils import (build_ddp, build_dp, default_device,
                            get_root_logger, register_module_hooks,
...
def parse_args():
    ...
    parser.add_argument('--diff_seed', default=False)
    parser.add_argument(
        '--deterministic',
        action='store_true',
...
def inference_pytorch_meta(args,
                           cfg,
                           distributed,
                           support_data_loader,
                           query_data_loader,
    ...
    if cfg.data.test.get('meta_test_cfg', None) is None:
        raise ValueError('meta_test_cfg should be specified in cfg.data.test')
...
def main():
    ...
    if args.eval:
        ...
        eval_config = Config._merge_a_into_b(
            dict(metric=args.eval), eval_config)
    ...
    if cfg.data.test['type'] == 'MetaTestDataset':
        dataset = MetaTestDataset(
            build_dataset(cfg.data.test['dataset']),
            num_episodes=cfg.data.test['num_episodes'],
            num_ways=cfg.data.test['num_ways'],
            num_shots=cfg.data.test['num_shots'],
            num_queries=cfg.data.test['num_queries'],
            episodes_seed=cfg.data.test.get('episodes_seed', None),
            subset=cfg.data.test.get('subset', None),
            train_labels=cfg.data.test.get('train_labels', None),
            val_labels=cfg.data.test.get('val_labels', None),
            test_labels=cfg.data.test.get('test_labels', None))
        if cfg.data.test.get('subset', None) == 'val':
        ...
        if args.tensorrt:
            raise NotImplementedError('TensorRT is not supported in '
                                      'meta-testing mode.')
        elif args.onnx:
            raise NotImplementedError('ONNX is not supported in '
                                      'meta-testing mode.')
        else:
            outputs = inference_pytorch_meta(
                args,
                cfg,
                distributed,
        ...
        if args.tensorrt:
            outputs = inference_tensorrt(args.checkpoint, distributed,
                                         data_loader,
