...
from mmaction.datasets import EpisodicDataset, build_dataset
...
def main():
    ...
    elif cfg.data.train['type'] == 'EpisodicDataset':
        is_fewshot = True
        dataset = EpisodicDataset(
            build_dataset(cfg.data.train['dataset']),
            num_episodes=cfg.data.train['num_episodes'],
            num_ways=cfg.data.train['num_ways'],
            num_shots=cfg.data.train['num_shots'],
            num_queries=cfg.data.train['num_queries'],
            episodes_seed=cfg.data.train.get('episodes_seed', None),
            subset=cfg.data.train.get('subset', None),
            train_labels=cfg.data.train.get('train_labels', None),
            val_labels=cfg.data.train.get('val_labels', None),
            test_labels=cfg.data.train.get('test_labels', None))
        datasets = [dataset]
    else:
    ...
    assert not (is_fewshot and (args.test_last or args.test_best)), \
        'test_last and test_best are not supported in fewshot setting'
