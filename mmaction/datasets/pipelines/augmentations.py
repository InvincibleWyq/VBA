...
class Imgaug:
    ...
    def __init__(self, transforms):
        ...
        if transforms == 'default':
            self.transforms = self.default_transforms(num_augmenters=2)
        elif transforms == 'default_1augs':
            self.transforms = self.default_transforms(num_augmenters=1)
    ...
    @staticmethod
    def default_transforms(num_augmenters=2):
...
class RandomErasing(object):
    ...
    def __call__(self, results):
        ...
        for frame in results['imgs']:
            frame = self._fill_pixels(frame, top, left, h, w)
            out_frame.append(frame)

        results['imgs'] = np.stack(out_frame, axis=0)
...
class Resize:
    ...
    def __call__(self, results):
        ...
        # import torch
        # torch.save(results['imgs'], 'imgs.pth')
        return results
