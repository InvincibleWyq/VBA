...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    Args:
        ...
        sample_by_class (bool): Sampling by class, should be set `True` when
            performing inter-class data balancing. Should also be set `True`
            when performing few shot learning. Only compatible with
    ...
    def parse_by_class(self):
        ...
        for idx, item in enumerate(self.video_infos):
            item['idx'] = idx
            label = item['label']
    ...
    def sample_shots_by_class_id(self, class_id=0, num_shots=2):
        """Random sample shots of given class id."""
        all_shot_ids = [
            video_info['idx']
            for video_info in self.video_infos_by_class[class_id]
        ]
