...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.out_block_indices = out_block_indices
    ...
    def forward(self, x):
        ...
        def _convert_to_3d_seq(x):
            """(N x T, C, H, W) -> (N, T, H x W, C)"""
            x = x.reshape(N, -1, x.shape[1], x.shape[2], x.shape[3]).flatten(3)
            x = x.permute(0, 1, 3, 2)
            return x
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            for block in res_layer:
                x = block(x)
                if block_id in self.out_block_indices:
                    x_duplicate = _convert_to_3d_seq(x.clone())
                    outs.append(x_duplicate)
                block_id += 1
