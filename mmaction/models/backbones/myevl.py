...
class EVLDecoder(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 12,
        mlp_factor: float = 4.0,
        temporal_conv_layer_index: List = [],
        temporal_pos_embed_layer_index: List = [],
        cross_frame_decoder_layer_index: List = [],
        mlp_dropout: float = 0.5,
    ):
        super().__init__()

        if temporal_conv_layer_index is None:
            temporal_conv_layer_index = []
        if temporal_pos_embed_layer_index is None:
            temporal_pos_embed_layer_index = []
        if cross_frame_decoder_layer_index is None:
            cross_frame_decoder_layer_index = []

        # assert no out of range
        assert all([x in range(num_layers) for x in temporal_conv_layer_index])
        assert all(
            [x in range(num_layers) for x in temporal_pos_embed_layer_index])
        assert all(
            [x in range(num_layers) for x in cross_frame_decoder_layer_index])

        # assert no repeated index
        assert len(
            set(temporal_conv_layer_index)) == len(temporal_conv_layer_index)
        assert len(set(temporal_pos_embed_layer_index)) == len(
            temporal_pos_embed_layer_index)
        assert len(set(cross_frame_decoder_layer_index)) == len(
            cross_frame_decoder_layer_index)

        self.temporal_conv_layer_index = temporal_conv_layer_index
        self.temporal_pos_embed_layer_index = temporal_pos_embed_layer_index
        self.cross_frame_decoder_layer_index = cross_frame_decoder_layer_index
        ...
        for i in self.temporal_conv_layer_index:
            self.temporal_conv[str(i)] = nn.Conv1d(
                in_feature_dim,
                in_feature_dim,
                kernel_size=3,
                stride=1,
                padding=1,
                groups=in_feature_dim)

        self.temporal_pos_embed = nn.ParameterDict()
        for i in self.temporal_pos_embed_layer_index:
            self.temporal_pos_embed[str(i)] = nn.Parameter(
                torch.zeros([num_frames, in_feature_dim]))

        self.cross_frame_decoder = nn.ModuleDict()
        for i in self.cross_frame_decoder_layer_index:
            self.cross_frame_decoder[str(i)] = TransformerDecoderLayer(
    ...
    def forward(self, in_features: List[Dict[str, torch.Tensor]]):
        ...
        x = self.cls_token.view(1, 1, 1, -1).repeat(N, T, 1, 1)

        for i in range(self.num_layers):
            frame_features = in_features[i]['out']  # N, T, L, C

            x = torch.cat([
                self.in_frame_decoder[i][t](
                    x[:, t, :, :], frame_features[:, t, :, :]).unsqueeze(1)
                for t in range(T)
            ],
                          dim=1)

            if i in self.temporal_pos_embed_layer_index:
                x += self.temporal_pos_embed[str(i)].view(1, T, 1, C)

            if i in self.temporal_conv_layer_index:
                x_temp = x
                # N * 1, C, T
                x_temp = x_temp.permute(0, 2, 3, 1).contiguous().flatten(0, 1)
                x_temp = self.temporal_conv[str(i)](x_temp)
                # N, T, 1, C
                x_temp = x_temp.view(N, 1, C, T).permute(0, 3, 1,
                                                         2).contiguous()
                x += x_temp

            if i in self.cross_frame_decoder_layer_index:
                # do self attention across T channel of x
                x = x.view(N, T, C)
                x = self.cross_frame_decoder[str(i)](x, x)
                x = x.view(N, T, 1, -1)
...
class MYEVL(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = EVLDecoder(
            ...
            temporal_conv_layer_index=temporal_conv_layer_index,
            temporal_pos_embed_layer_index=temporal_pos_embed_layer_index,
            cross_frame_decoder_layer_index=cross_frame_decoder_layer_index,
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.decoder(features).squeeze(2)
        x = self.layer_norm(x)
        return x
